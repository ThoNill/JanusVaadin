...
public class StageConnector extends VaadinBasisConnector implements
    ...
    private static final Logger LOG = LogManager
            .getLogger(StageConnector.class);
    ...
    /*
     * @Override public void addComponent(GuiComponent comp) { if (comp
     * instanceof MenuBarConnector) { setMenuBarConnector((MenuBarConnector)
     * comp); return; } if (comp instanceof ContextMenuConnector) { ContextMenu
     * contextMenu = ((ContextMenuConnector) comp).getMenu();
     * 
     * pane.addEventHandler(ContextMenuEvent.CONTEXT_MENU_REQUESTED, event -> {
     * contextMenu.show(pane, event.getScreenX(), event.getScreenY());
     * event.consume(); }); pane.addEventHandler(MouseEvent.MOUSE_PRESSED, event
     * -> { contextMenu.hide(); }); return; } if (comp instanceof
     * VaadinBasisConnector) { VaadinBasisConnector childConnector =
     * (VaadinBasisConnector) comp;
